using System.Net;
using CommonTestUtilities.Requests;
using FluentAssertions;

namespace WebApi.Test.Platform.Register;

public class RegisterPlatformInvalidTokenTest : CandidatusClassFixture
{
    private readonly string METHOD = "platform";
    public RegisterPlatformInvalidTokenTest(CustomWebApplicationFactory factory) : base(factory)
    {
    }

    [Fact]
    public async Task Success()
    {
        var request = RequestRegisterPlatformJsonBuilder.Build();
        var response = await DoPost(METHOD, request, "invalidToken");

        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
}